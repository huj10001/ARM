...
static uint8_t black_list [11] = {0};
static int prev_parent = -1;
...
send_packet(void *ptr)
{
    ...
    int sizeof_buf = sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint8_t) 
		    ...
		    + num * ( sizeof(uint8_t) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint16_t));
	...
	//rank = rpl_get_parent_rank(uip_ds6_nbr_get_ll(nbr));
        ...
        // rpl_parent_t *p_nbr;
        // uip_ipaddr_t *parent_ipaddr_nbr;
        // p_nbr = rpl_find_parent(current_dag, &nbr->ipaddr);
        // parent_ipaddr_nbr =rpl_get_parent_ipaddr(p_nbr);

        nbr->rank = p->rank;
        nbr->etx = p->mc.obj.etx;
        // nbr->link_metric = p->link_metric;
        // nbr->link_metric = calculate_path_metric(p);
        MAPPER_ADD_PACKETDATA(buf_p,p->rank);
        MAPPER_ADD_PACKETDATA(buf_p,nbr->rank_fake);
        MAPPER_ADD_PACKETDATA(buf_p,nbr->etx_fake);
        printf("neighbor : %02x rank %u etx %u\n",nbr->ipaddr.u8[15],nbr->rank,nbr->etx);
        // printf("neighbor : %02x rank %u etx %u\n",nbr->ipaddr.u8[15],nbr->rank, nbr->link_metric/RPL_DAG_MC_ETX_DIVISOR);


        /***** monitor node 7,10 *****/
        /*
        if(first_etx==0){
            first_etx = nbr->link_metric/RPL_DAG_MC_ETX_DIVISOR;
            // printf("first etx is %u.\n", first_etx);
        }else{
            if(second_etx == 0){
                second_etx = nbr->link_metric/RPL_DAG_MC_ETX_DIVISOR;
                // printf("second etx is %u.\n", second_etx);

                if(relation == -1){
                    relation = (first_etx - second_etx)>0;
                    printf("initialize the relation %u.\n", relation);
                }else{
                    int current_relation = (first_etx- second_etx)>0;
                    
                    printf("current relation is %u.\n", current_relation);
                    if(current_relation != relation){
                        if(counter < 3){
                            counter ++;
                            printf("relation is broken, counter %u.\n", counter);
                        }else{
                            printf("reach the thredhold,ALARM!\n");
                        }
                    }
                    else{
                        printf("relation is valid, refresh.\n");
                    }
                }
            }
            else{
